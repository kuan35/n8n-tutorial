...
```bash
nano ~/n8n-local/docker-compose.yml
```
...
2. 設定：
   ...
   - **Model**：`gemini-2.5-flash`（或 `gemini-1.5-flash`）
...
4. 連接：**Loop Over Items**（Loop 端口）→ **GET YouTube留言**
...
> **注意**：第 7 行的 `UCRc0GUnMU00Qiwc-aNWIwww` 換成你自己的 YouTube 頻道 ID（在 YouTube Studio → 設定 → 頻道 → 基本資訊可找到）。
